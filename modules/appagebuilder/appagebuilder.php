    ...
    public function renderForm()
    {
        ...
        $form_general = array(
            ...
            'input' => array(
                ...
                array(
                    'type' => 'switch',
                    'label' => $this->l('Enable slide images'),
                    'name' => 'APPAGEBUILDER_SLIDE_IMAGE',
                    'default' => 1,
                    'desc' => $this->l('Display slide product on mobile'),
                    'values' => ApPageSetting::returnYesNo(),
                ),
                array(
                    'type' => 'text',
...
//            $order_by_prefix = 'cp';
    ...
    //            $sql .= ' ORDER BY product_shop.date_add '.(!$get_total ? ' LIMIT '.(int)$n : '');
        ...
        foreach($result as &$product) {
            ...
            $tax = Tax::getProductTaxRate($product['id_product']);

            // Get the Attributes
            $groups = array();
            $context = Context::getContext();

            $attributes_groups = $objProduct->getAttributesGroups($context->language->id);
            if (is_array($attributes_groups) && $attributes_groups) {
                ...
                $product['good_price'] = 0;
                if( count($groups) > 0 ) {
                    $price = (float)$groups[count($groups) -1 ]['price'];

                    $str = $groups[count($groups) -1 ]['attribute_name'];
                    ...
                    // echo $price . "___" .$unit . "_________";

                    $priceFormatter = new PriceFormatter();
                    if ($unit != 0) {
                        $product['good_price'] = $priceFormatter->convertAndFormat($price * (1 + (float)$tax / 100) / $unit);
                        $product['has_good_price'] = 1;
                    }
                    else {
                        $product['good_price'] = 0;
                        $product['has_good_price'] = 0;
                    }
                    // echo $product['good_price'] . "_________";
                }
            }
        }
        // var_dump($result);
...
//        }
    ...
    protected function getCacheId($hookName = null, $shortcode_key = null)
    {
        ...
        //process nomal cache for each hook
            ...
            $id_profile = Tools::getIsset('id_appagebuilder_profiles') ? Tools::getValue('id_appagebuilder_profiles') : false;
            if ($id_profile !== false) {
                $cache_array[] = 'profile_'.$id_profile;
            }
            else
            {
                ...
                $cache_array[] = 'profile_'.$linkRewrive;
            }
            ...
            if ($this->profile_param && isset($this->profile_param[$hookName]) && $this->profile_param[$hookName]) {
                ...
                } elseif (isset($this->profile_param[$hookName]['productCarousel'])) {
                    $random = round(rand(1, max(Configuration::get('APPAGEBUILDER_PRODUCT_MAX_RANDOM'), 1)));
                    $cache_array[] = "p_carousel_$random";
                } else if (isset($this->profile_param[$hookName]['exception']) && in_array($cache_array, $this->profile_param[$hookName]['exception'])) {
        ...
        if ($disable_cache)
        {
            ...
            if ($is_live) {
                ...
                $cover_hook_live = '<div class="cover-hook"><a title="'.$this->l('Click to edit').'" class="lnk-hook" href="'
                        .$url_design_layout.'#'.$hookName.'" target="_blank">Hook: '.Tools::strtoupper($hookName).'</a></div>';
                $ap_content = '<div class="ap-cover-hook">'.$ap_content.'</div>';
            }

            $this->smarty->assign(array('apContent' => $ap_content));
            return $cover_hook_live.$this->fetch( $this->templateFile);
        }else {
    ...
    public function hookDisplayLeoProfileProduct($params)
    {
        ...
        if (!isset($params['product'])) {
            ...
            $tpl_file = _PS_ALL_THEMES_DIR_.apPageHelper::getThemeName().'/' . $params['load_file'];
