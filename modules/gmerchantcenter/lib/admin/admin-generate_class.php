        ...
        try {
            foreach (self::$aParamsForXml as $sParamName) {
                ...
                if (Tools::getValue($sParamName) !== false) {
                    ...
                    throw new Exception(GMerchantCenter::$oModule->l('One or more of the required parameters are not provided, please check the list in the current class',
                            'admin-generate_class') . '.', 800);
                }
            }
    ...
    private function generateFlyOutput(array $aPost = null)
    {
        ...
        try {
            ...
            if (!empty(GMerchantCenter::$conf['GMC_FEED_TOKEN'])
                ...
                throw new Exception(GMerchantCenter::$oModule->l('Invalid security token',
                        'admin-generate_class') . '.', 810);
            }
    ...
    private function generateCron(array $aPost = null)
    {
        ...
        try {
            ...
            if (empty($aContent['assign']['aErrors'])) {
                // handle the cron URL
                $sCronUrl = GMerchantCenter::$conf['GMC_LINK'] . _GMC_MODULE_URL . 'cron.php?id_shop=' . $_POST['iShopId'];
