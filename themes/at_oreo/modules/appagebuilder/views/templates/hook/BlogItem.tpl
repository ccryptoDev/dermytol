    ...
    <div class="right-block">
    	<div class="blog-meta">
			{if isset($formAtts.bleoblogs_saut) && $formAtts.bleoblogs_saut}
				<span class="author">
					<i class="icon-font icon-user-following"></i> {$blog.author|escape:'html':'UTF-8'}
				</span>
			{/if}		
			...
			{if isset($formAtts.bleoblogs_scoun) && $formAtts.bleoblogs_scoun}
				<span class="nbcomment">
					<i class="icon-font icon-speech"></i> {$blog.comment_count|intval}
				</span>
			{/if}
        ...
        {if isset($formAtts.show_title) && $formAtts.show_title}
        	<h5 class="blog-title" itemprop="name"><a href="{$blog.link}{*full url can not escape*}" title="{$blog.title|escape:'html':'UTF-8'}">{$blog.title|strip_tags:'UTF-8'|truncate:160:'...'}</a></h5>
        {/if}
